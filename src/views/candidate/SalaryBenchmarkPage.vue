...
async function fetchBenchmark(): Promise<void> {
 ...
 ui.toastWarning('Job title required', 'Enter a job title to look up salary data.')
 return
 }
 loading.value = true
...
<template>
 ...
 <div class="mb-8">
 <h1 class="text-3xl font-extrabold text-slate-900 mb-2">Salary Reference</h1>
 <p class="text-sm font-medium text-slate-500">AI-powered salary analysis and market data for any position.</p>
 </div>
 ...
 {{ loading ? 'Looking up…' : 'View Salary' }}
 </button>
 </form>
 </div>
 ...
 <div class="h-8 bg-slate-200 rounded w-64" />
 ...
 <h2 class="text-2xl font-black text-slate-900 mb-2">{{ benchmark.jobTitle }}</h2>
 ...
 <!-- Median marker -->
 <div
 v-if="benchmark.range.max != null && benchmark.range.min != null && benchmark.range.median != null && benchmark.range.max !== benchmark.range.min"
 ...
 <div v-else class="py-4 text-center text-sm font-medium text-slate-400">No salary data available for this position.</div>
 ...
 <h3 class="text-lg font-bold text-slate-900 mb-4">Market Analysis</h3>
 ...
 <span class="text-sm font-medium text-slate-700 leading-relaxed">{{ insight }}</span>
 </li>
 </ul>
 </div>
 ...
 <p class="text-[10px] font-bold text-slate-400 uppercase tracking-wider">Updated at {{ formatDate(benchmark.generatedAt) }}</p>
 </div>
 </div>
 ...
 <p class="text-sm font-medium text-slate-500 max-w-md mx-auto">We will analyze market salary data using AI and real job listings.</p>
 </div>
 </div>
</template>
