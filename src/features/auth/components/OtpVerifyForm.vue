...
async function handleSubmit(): Promise<void> {
  ...
  if (!validate()) return;
  try {
    // If the method signature doesn't match returning success vs failure, we should handle via exact API check.
    // However, verifyOtp throws on error in store. So we just await.
    await auth.verifyOtp({ email: props.email, code: code.value });
    emit("success");
  } catch (err: any) {
    // Error is handled in auth store but we must catch to not emit success
  }
}
...
async function handleResend(): Promise<void> {
  const success = await auth.resendOtp(props.email);
