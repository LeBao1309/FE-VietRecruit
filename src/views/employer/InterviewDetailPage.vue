  ...
  <div class="max-w-4xl mx-auto px-6 py-8">
    ...
    <div v-if="interviewStore.detailLoading" class="space-y-6">
      <div class="premium-card p-8 animate-pulse space-y-5">
        <div class="h-8 bg-slate-100 dark:bg-slate-800 rounded-lg w-72" />
        <div class="h-5 bg-slate-100 dark:bg-slate-800 rounded-lg w-56" />
        <div class="h-24 bg-slate-100 dark:bg-slate-800 rounded-xl w-full" />
      </div>
    </div>
    ...
    <div v-else-if="interview" class="space-y-6">
      ...
      <div class="premium-card p-8 relative overflow-hidden">
        ...
        <div class="flex flex-col sm:flex-row items-baseline sm:items-start justify-between gap-6 mb-6 relative">
          <div>
            <h1 class="text-2xl font-extrabold text-slate-900 dark:text-white mb-3">{{ interview.title }}</h1>
        ...
        <!-- Details grid -->
        <div class="grid grid-cols-2 sm:grid-cols-4 gap-6 pt-6 border-t border-slate-100 dark:border-slate-700 mt-2">
          <div>
            <span class="block text-[10px] font-bold uppercase tracking-wider text-slate-400 mb-1.5">Scheduled</span>
            <span class="text-sm font-bold text-slate-800 dark:text-slate-200">{{ formatDateTime(interview.scheduledAt) }}</span>
          </div>
          <div>
            <span class="block text-[10px] font-bold uppercase tracking-wider text-slate-400 mb-1.5">Duration</span>
            <span class="text-sm font-bold text-slate-800 dark:text-slate-200">{{ formatDuration(interview.durationMinutes) }}</span>
          </div>
          <div>
            <span class="block text-[10px] font-bold uppercase tracking-wider text-slate-400 mb-1.5">Type</span>
            <span class="text-sm font-bold text-slate-800 dark:text-slate-200">{{ interview.interviewType ?? '—' }}</span>
          </div>
          <div>
            ...
            <a
              ...
              target="_blank"
              rel="noopener"
              class="text-sm text-teal-600 hover:text-teal-700 dark:text-teal-400 dark:hover:text-teal-300 font-bold transition-colors break-all"
            >
              {{ interview.locationOrLink }}
            </a>
            <span v-else class="text-sm font-bold text-slate-800 dark:text-slate-200">{{ interview.locationOrLink ?? '—' }}</span>
          </div>
        </div>

        <!-- Back link (context-aware) -->
        <div class="mt-6 pt-5 border-t border-slate-100 dark:border-slate-700 flex items-center gap-4">
          <router-link
      ...
      <div class="premium-card p-8">
        ...
        <div class="space-y-3">
          <div
            ...
            :key="iv.id"
            class="flex items-center gap-4 p-4 bg-slate-50 dark:bg-slate-800/50 rounded-xl border border-slate-200 dark:border-slate-700 transition hover:bg-slate-100 dark:hover:bg-slate-800"
          >
            <div class="w-10 h-10 rounded-full bg-teal-100 dark:bg-teal-900/30 text-teal-700 dark:text-teal-400 flex items-center justify-center text-sm font-bold shrink-0">
              {{ iv.fullName.charAt(0).toUpperCase() }}
            </div>
            <div class="flex-1 min-w-0">
              <span class="text-sm font-bold text-slate-900 dark:text-white block">{{ iv.fullName }}</span>
      ...
      <div class="premium-card p-8">
        ...
        <div v-if="interviewStore.scorecardsLoading" class="animate-pulse space-y-4">
          <div v-for="i in 2" :key="i" class="h-24 bg-slate-100 dark:bg-slate-800 rounded-xl" />
        </div>
        ...
        <div v-else class="space-y-4">
          <div
            ...
            :key="sc.id"
            class="bg-slate-50 dark:bg-slate-800/50 border border-slate-200 dark:border-slate-700 rounded-2xl p-5"
          >
            <!-- Header -->
      ...
      <div class="premium-card p-8 border-t-4 border-t-amber-400">
        <div class="flex items-center justify-between mb-6">
          <h2 class="text-lg font-bold text-slate-900 dark:text-white">AI Interview Questions</h2>
          <div class="flex items-center gap-3">
            <button
              v-if="!interviewStore.aiQuestions"
              @click="loadQuestions"
              class="text-xs text-slate-500 hover:text-slate-700 dark:text-slate-400 dark:hover:text-slate-200 font-bold transition-colors uppercase tracking-wider"
            >
              Load Questions
            </button>
            <button
      ...
      <div v-if="showConfirm && confirmAction" class="premium-modal-backdrop">
        ...
        <div class="premium-modal-content w-full max-w-sm text-center">
          <div
            ...
            :class="confirmAction === 'complete' ? 'bg-emerald-50 text-emerald-500 border-emerald-100 dark:border-emerald-900' : 'bg-slate-100 text-slate-500 border-slate-200 dark:bg-slate-800 dark:border-slate-700'"
          >
