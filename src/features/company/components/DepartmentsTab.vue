...
const handleSubmit = () => {
  if (currentDepartment.value) {
    emit('update', currentDepartment.value.id, { ...formData.value })
  } else {
...
const confirmDelete = () => {
  if (currentDepartment.value) {
    emit('delete', currentDepartment.value.id)
...
<template>
  <div class="space-y-4">
    <div class="flex justify-between items-center">
      <h3 class="text-lg font-bold text-gray-800">Danh sách Phòng ban</h3>
      <button
        @click="openModal()"
      ...
      <div class="overflow-x-auto">
        <table class="w-full text-left border-collapse">
          <thead class="bg-gray-50 border-b border-gray-200">
            <tr>
              <th class="px-6 py-4 text-xs font-semibold text-gray-500 uppercase tracking-wider">Tên phòng ban</th>
      ...
      <div class="bg-white rounded-xl shadow-xl max-w-md w-full overflow-hidden">
        ...
        <div class="p-6 space-y-4">
          <div class="space-y-1.5">
            <label class="text-sm font-semibold text-gray-700">Tên phòng ban <span class="text-red-500">*</span></label>
            <input
              ...
              placeholder="Ví dụ: Kỹ thuật, Nhân sự..."
            />
          </div>
          <div class="space-y-1.5">
            <label class="text-sm font-semibold text-gray-700">Mô tả (Tùy chọn)</label>
            <textarea
              v-model="formData.description"
              rows="3"
              class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-[#009898]/20 focus:border-[#009898] outline-none resize-none"
    ...
    <div v-if="isDeleteConfirmOpen" class="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/50">
      <div class="bg-white rounded-xl shadow-xl max-w-sm w-full p-6 text-center">
        ...
        <p class="text-gray-600 mb-6">
          Bạn có chắc chắn muốn xóa phòng ban <strong>{{ currentDepartment?.name }}</strong>? Hành động này không thể hoàn tác.
        </p>
