...
<template>
  <div v-if="hasError" class="min-h-screen flex items-center justify-center bg-slate-50 dark:bg-slate-900 p-6">
    <div class="max-w-md w-full premium-card p-8 text-center relative overflow-hidden">
      ...
      <h1 class="text-xl font-extrabold text-slate-900 dark:text-white mb-2">Something went wrong</h1>
      <p class="text-sm font-medium text-slate-500 mb-6">
        An unexpected error occurred. Please try reloading the page.
      </p>
      
      <!-- Optional: Display error details in development mode -->
      <div v-if="errorMessage" class="mb-6 text-xs text-left text-rose-600 dark:text-rose-400 bg-rose-50 dark:bg-rose-900/20 p-4 rounded-xl overflow-auto max-h-32 border border-rose-100 dark:border-rose-900/50">
        {{ errorMessage }}
      </div>
