...
import { CategoryRequestSchema } from '@/features/company/types/org.dto'
...
async function handleSave() {
  ...
  let ok = false
  if (isEdit.value) {
    ok = await store.updateCategory(currentId.value, result.data)
  } else {
    ok = await store.createCategory(result.data)
  }
...
function nextPage() {
  if (store.catPage < store.catTotalPages - 1) {
    store.fetchCategories(store.catPage + 1)
  }
}
...
<template>
  <div class="card space-y-4">
    ...
    <div class="flex justify-between items-center mb-4">
      <h2 class="text-lg font-semibold text-text-primary">Danh mục</h2>
    ...
    <div class="overflow-x-auto border border-surface-muted rounded-lg">
      <table class="w-full text-left text-sm text-text-secondary">
        <thead class="bg-surface-soft text-text-primary border-b border-surface-muted">
          <tr>
            <th class="px-4 py-3 font-medium">Tên danh mục</th>
        ...
        <tbody class="divide-y divide-surface-muted">
          <tr v-if="store.isLoading" class="animate-pulse">
            <td colspan="2" class="px-4 py-4 text-center">Đang tải...</td>
          </tr>
          <tr v-else-if="store.categories.length === 0">
            <td colspan="2" class="px-4 py-4 text-center">Không có dữ liệu</td>
          </tr>
          <tr v-else v-for="cat in store.categories" :key="cat.id" class="hover:bg-surface-soft/50">
    ...
    <div class="flex items-center justify-between pt-4">
      <div class="text-sm text-text-secondary">
        Page {{ store.catTotalPages > 0 ? store.catPage + 1 : 0 }} / {{ store.catTotalPages }} (Tổng: {{ store.catTotalElements }})
      </div>
      <div class="space-x-2 flex">
        <button 
          ...
          @click="prevPage" 
          :disabled="store.catPage === 0 || store.isLoading"
        >
          Prev
        </button>
        <button 
          class="px-3 py-1.5 border border-surface-muted rounded text-sm disabled:opacity-50 disabled:cursor-not-allowed hover:bg-surface-soft transition-colors"
          @click="nextPage" 
          :disabled="store.catPage >= store.catTotalPages - 1 || store.isLoading"
        >
          Next
        </button>
      </div>
    ...
    <div v-if="showModal" class="fixed inset-0 z-50 flex items-center justify-center overflow-y-auto bg-black/50">
      <div class="bg-surface-base w-full max-w-md rounded-xl p-6 shadow-xl mx-4 my-8">
        <h3 class="text-lg font-semibold text-text-primary mb-4">{{ isEdit ? 'Sửa Danh mục' : 'Thêm Danh mục' }}</h3>
