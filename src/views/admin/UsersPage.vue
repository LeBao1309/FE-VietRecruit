...
function getRoleBadgeClass(role: string): string {
  switch (role) {
    case 'SYSTEM_ADMIN': return 'bg-amber-50 text-amber-700 border-amber-200 dark:bg-amber-500/10 dark:text-amber-400 dark:border-amber-500/20'
    case 'COMPANY_ADMIN': return 'bg-blue-50 text-blue-700 border-blue-200 dark:bg-blue-500/10 dark:text-blue-400 dark:border-blue-500/20'
    case 'HR': return 'bg-indigo-50 text-indigo-700 border-indigo-200 dark:bg-indigo-500/10 dark:text-indigo-400 dark:border-indigo-500/20'
    case 'INTERVIEWER': return 'bg-purple-50 text-purple-700 border-purple-200 dark:bg-purple-500/10 dark:text-purple-400 dark:border-purple-500/20'
    case 'CANDIDATE': return 'bg-emerald-50 text-emerald-700 border-emerald-200 dark:bg-emerald-500/10 dark:text-emerald-400 dark:border-emerald-500/20'
    case 'CUSTOMER_SERVICE': return 'bg-pink-50 text-pink-700 border-pink-200 dark:bg-pink-500/10 dark:text-pink-400 dark:border-pink-500/20'
...
function getStatusIndicator(user: AdminUserResponse): { text: string; cls: string } {
  if (user.isLocked) return { text: 'Locked', cls: 'bg-rose-50 text-rose-600 border-rose-200 dark:bg-rose-500/10 dark:text-rose-400 dark:border-rose-500/20' }
  if (!user.isActive) return { text: 'Inactive', cls: 'bg-slate-50 text-slate-600 border-slate-200 dark:bg-slate-500/10 dark:text-slate-400 dark:border-slate-500/20' }
  return { text: 'Active', cls: 'bg-emerald-50 text-emerald-600 border-emerald-200 dark:bg-emerald-500/10 dark:text-emerald-400 dark:border-emerald-500/20' }
}
...
<template>
  <div class="max-w-6xl mx-auto px-6 py-10">
    ...
    <div class="flex flex-col sm:flex-row sm:items-end justify-between gap-4 mb-8">
      <div>
        <h1 class="text-2xl font-extrabold text-slate-900 dark:text-white">User Management</h1>
    ...
    <div v-else-if="admin.users && admin.users.empty" class="premium-card p-16 text-center">
      ...
      <h2 class="text-lg font-bold text-slate-900 dark:text-white mb-2">No Users Found</h2>
    ...
    <template v-else-if="admin.users">
      <div class="premium-card overflow-hidden">
        <div class="overflow-x-auto">
          <table class="w-full text-left border-collapse whitespace-nowrap">
            ...
            <tbody>
              <tr v-for="user in admin.userList" :key="user.id" class="border-b border-slate-100 dark:border-slate-800 hover:bg-slate-50 dark:hover:bg-slate-800/50 transition-colors group">
                <td class="px-6 py-4">
                  <div class="flex items-center gap-3">
                    <div class="w-8 h-8 rounded-full overflow-hidden bg-slate-100 dark:bg-slate-800 flex items-center justify-center shrink-0">
                    ...
                    <span class="text-sm font-bold text-slate-900 dark:text-white">{{ user.fullName }}</span>
                  </div>
                </td>
                ...
                <td class="px-6 py-4">
                  <div class="flex items-center justify-center gap-1 opacity-0 group-hover:opacity-100 transition-opacity">
                    <button class="p-2 text-slate-400 hover:text-teal-600 transition-colors rounded-lg hover:bg-teal-50 dark:hover:bg-teal-500/10" title="View" @click="openDetail(user)">
                      <svg class="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M15 12a3 3 0 11-6 0 3 3 0 016 0z" /><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M2.458 12C3.732 7.943 7.523 5 12 5c4.478 0 8.268 2.943 9.542 7-1.274 4.057-5.064 7-9.542 7-4.477 0-8.268-2.943-9.542-7z" /></svg>
                    </button>
                    <button class="p-2 text-slate-400 hover:text-blue-600 transition-colors rounded-lg hover:bg-blue-50 dark:hover:bg-blue-500/10" title="Edit" @click="openEdit(user)">
                      <svg class="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M15.232 5.232l3.536 3.536m-2.036-5.036a2.5 2.5 0 113.536 3.536L6.5 21.036H3v-3.572L16.732 3.732z" /></svg>
                    </button>
                    <button class="p-2 text-slate-400 hover:text-rose-600 transition-colors rounded-lg hover:bg-rose-50 dark:hover:bg-rose-500/10" title="Delete" @click="confirmDelete(user)">
      ...
      <div v-if="totalPages > 1" class="flex flex-col sm:flex-row items-center justify-between gap-4 mt-8 pt-6 border-t border-slate-200 dark:border-slate-800">
        <span class="text-sm font-bold text-slate-500 order-2 sm:order-1">
          Page {{ currentPage + 1 }} of {{ totalPages }} <span class="mx-1 text-slate-300 dark:text-slate-600">·</span> {{ admin.users.totalElements }} total
        </span>
        <div class="flex items-center gap-2 order-1 sm:order-2">
          <button class="px-4 py-2 text-sm font-bold text-slate-600 dark:text-slate-300 border border-slate-200 dark:border-slate-700 rounded-xl hover:bg-slate-50 dark:hover:bg-slate-800 disabled:opacity-40 disabled:cursor-not-allowed transition-colors" :disabled="admin.users.first" @click="goToPage(currentPage - 1)">
            &larr; Previous
          </button>
          <button class="px-4 py-2 text-sm font-bold text-slate-600 dark:text-slate-300 border border-slate-200 dark:border-slate-700 rounded-xl hover:bg-slate-50 dark:hover:bg-slate-800 disabled:opacity-40 disabled:cursor-not-allowed transition-colors" :disabled="admin.users.last" @click="goToPage(currentPage + 1)">
            Next &rarr;
          </button>
        </div>
      </div>
      ...
      <div v-if="showCreateModal" class="fixed inset-0 z-50 flex items-center justify-center">
        ...
        <div class="relative premium-card w-full max-w-lg p-6 sm:p-8 animate-slide-up mx-4 shadow-2xl overflow-y-auto max-h-[90vh]">
          <div class="flex items-center justify-between mb-6">
            <h2 class="text-xl font-extrabold text-slate-900 dark:text-white">Create User</h2>
          ...
          <form @submit.prevent="submitCreate" class="space-y-5">
            <div>
              <label class="block text-sm font-bold text-slate-700 dark:text-slate-300 mb-1.5">Full Name <span class="text-rose-500">*</span></label>
              <input v-model="form.fullName" type="text" required maxlength="255" placeholder="Full name" class="w-full px-4 py-3 border border-slate-200 dark:border-slate-700 rounded-xl bg-slate-50 dark:bg-slate-800/50 outline-none focus:border-teal-500 focus:ring-4 focus:ring-teal-500/10 transition-all font-medium" />
            </div>
            <div>
              <label class="block text-sm font-bold text-slate-700 dark:text-slate-300 mb-1.5">Email</label>
              <input v-model="form.email" type="email" maxlength="255" placeholder="Email address" class="w-full px-4 py-3 border border-slate-200 dark:border-slate-700 rounded-xl bg-slate-50 dark:bg-slate-800/50 outline-none focus:border-teal-500 focus:ring-4 focus:ring-teal-500/10 transition-all font-medium" />
            </div>
            <div class="grid grid-cols-1 sm:grid-cols-2 gap-5">
              <div>
                <label class="block text-sm font-bold text-slate-700 dark:text-slate-300 mb-1.5">Phone</label>
                <input v-model="form.phone" type="text" maxlength="20" placeholder="+84 …" class="w-full px-4 py-3 border border-slate-200 dark:border-slate-700 rounded-xl bg-slate-50 dark:bg-slate-800/50 outline-none focus:border-teal-500 focus:ring-4 focus:ring-teal-500/10 transition-all font-medium" />
              </div>
              <div>
                <label class="block text-sm font-bold text-slate-700 dark:text-slate-300 mb-1.5">Gender</label>
                <select v-model="form.gender" class="w-full px-4 py-3 border border-slate-200 dark:border-slate-700 rounded-xl bg-slate-50 dark:bg-slate-800/50 outline-none focus:border-teal-500 focus:ring-4 focus:ring-teal-500/10 transition-all font-medium appearance-none">
                  <option value="">—</option>
              ...
              <div>
                <label class="block text-sm font-bold text-slate-700 dark:text-slate-300 mb-1.5">Date of Birth</label>
                <input v-model="form.dob" type="date" class="w-full px-4 py-3 border border-slate-200 dark:border-slate-700 rounded-xl bg-slate-50 dark:bg-slate-800/50 outline-none focus:border-teal-500 focus:ring-4 focus:ring-teal-500/10 transition-all font-medium" />
              </div>
              <div>
                <label class="block text-sm font-bold text-slate-700 dark:text-slate-300 mb-1.5">Location</label>
                <input v-model="form.location" type="text" maxlength="255" placeholder="City, Country" class="w-full px-4 py-3 border border-slate-200 dark:border-slate-700 rounded-xl bg-slate-50 dark:bg-slate-800/50 outline-none focus:border-teal-500 focus:ring-4 focus:ring-teal-500/10 transition-all font-medium" />
              </div>
            </div>
            <div class="flex justify-end gap-3 pt-6 border-t border-slate-100 dark:border-slate-800">
              <button type="button" class="px-5 py-2.5 text-sm font-bold text-slate-600 dark:text-slate-300 bg-white dark:bg-slate-800 border border-slate-200 dark:border-slate-700 rounded-xl hover:bg-slate-50 dark:hover:bg-slate-700 transition-colors" @click="showCreateModal = false">Cancel</button>
    ...
    <Teleport to="body">
      <div v-if="showEditModal" class="fixed inset-0 z-50 flex items-center justify-center">
        ...
        <div class="relative premium-card w-full max-w-lg p-6 sm:p-8 animate-slide-up mx-4 shadow-2xl overflow-y-auto max-h-[90vh]">
          <div class="flex items-center justify-between mb-6">
            <h2 class="text-xl font-extrabold text-slate-900 dark:text-white">Edit User</h2>
          ...
          <form @submit.prevent="submitEdit" class="space-y-5">
            ...
            <div class="flex justify-end gap-3 pt-6 border-t border-slate-100 dark:border-slate-800">
              <button type="button" class="px-5 py-2.5 text-sm font-bold text-slate-600 dark:text-slate-300 bg-white dark:bg-slate-800 border border-slate-200 dark:border-slate-700 rounded-xl hover:bg-slate-50 dark:hover:bg-slate-700 transition-colors" @click="showEditModal = false">Cancel</button>
              <button type="submit" class="btn-primary py-2.5 px-6 min-w-[120px] flex justify-center items-center gap-2" :disabled="admin.userLoading || !form.fullName">
                <span v-if="admin.userLoading" class="w-4 h-4 rounded-full border-2 border-white/40 border-t-white animate-spin"></span>
    ...
    <Teleport to="body">
      <div v-if="showDetailModal && targetUser" class="fixed inset-0 z-50 flex items-center justify-center">
        ...
        <div class="relative premium-card w-full max-w-2xl p-6 sm:p-8 animate-slide-up mx-4 shadow-2xl overflow-y-auto max-h-[90vh]">
          <div class="flex items-center justify-between mb-6">
            <h2 class="text-xl font-extrabold text-slate-900 dark:text-white">User Details</h2>
          ...
          <div class="space-y-6">
            <!-- Profile Header -->
            <div class="flex flex-col sm:flex-row items-center gap-5 pb-6 border-b border-slate-100 dark:border-slate-800">
              <div class="w-20 h-20 rounded-full overflow-hidden bg-slate-100 dark:bg-slate-800 flex items-center justify-center shrink-0 border-4 border-white dark:border-slate-900 shadow-sm">
              ...
              <div class="text-center sm:text-left">
                <h3 class="text-xl font-extrabold text-slate-900 dark:text-white">{{ targetUser.fullName }}</h3>
                <p class="text-sm font-medium text-slate-500 bg-slate-100 dark:bg-slate-800 px-3 py-1 rounded-full inline-block mt-1">{{ targetUser.email }}</p>
            ...
            <div class="grid grid-cols-1 sm:grid-cols-2 gap-y-4 gap-x-8 text-sm">
              ...
              <div class="flex flex-col">
                ...
                <span class="font-medium text-slate-900 dark:text-white">{{ targetUser.phone ?? '—' }}</span>
              </div>
              <div class="flex flex-col">
                <span class="text-xs font-bold text-slate-400 uppercase tracking-wider mb-1">Location</span>
                <span class="font-medium text-slate-900 dark:text-white">{{ targetUser.location ?? '—' }}</span>
              </div>
              <div class="flex flex-col">
                <span class="text-xs font-bold text-slate-400 uppercase tracking-wider mb-1">Gender</span>
                <span class="font-medium text-slate-900 dark:text-white">{{ targetUser.gender ?? '—' }}</span>
              </div>
              <div class="flex flex-col">
                <span class="text-xs font-bold text-slate-400 uppercase tracking-wider mb-1">Date of Birth</span>
                <span class="font-medium text-slate-900 dark:text-white">{{ targetUser.dob ?? '—' }}</span>
              </div>
              <div class="flex flex-col">
                <span class="text-xs font-bold text-slate-400 uppercase tracking-wider mb-1">Company ID</span>
                <span class="font-medium text-slate-900 dark:text-white font-mono text-xs">{{ targetUser.companyId ?? '—' }}</span>
              </div>
              <div class="flex flex-col">
                <span class="text-xs font-bold text-slate-400 uppercase tracking-wider mb-1">Failed Attempts</span>
                <span class="font-medium text-slate-900 dark:text-white">{{ targetUser.failedAttempts }}</span>
              </div>
              <div class="flex flex-col">
                <span class="text-xs font-bold text-slate-400 uppercase tracking-wider mb-1">Lock Until</span>
                <span class="font-medium text-slate-900 dark:text-white">{{ formatDate(targetUser.lockUntil) }}</span>
              </div>
              <div class="flex flex-col">
                <span class="text-xs font-bold text-slate-400 uppercase tracking-wider mb-1">Last Login</span>
                <span class="font-medium text-slate-900 dark:text-white">{{ formatDate(targetUser.lastLoginAt) }}</span>
              </div>
              <div class="flex flex-col">
                <span class="text-xs font-bold text-slate-400 uppercase tracking-wider mb-1">Created</span>
                <span class="font-medium text-slate-900 dark:text-white">{{ formatDate(targetUser.createdAt) }}</span>
              </div>
            </div>
            
            <div v-if="targetUser.linkedinUrl || targetUser.githubUrl || targetUser.portfolioUrl" class="pt-4 border-t border-slate-100 dark:border-slate-800 grid grid-cols-1 sm:grid-cols-2 gap-4">
          ...
          <div class="flex justify-end gap-3 pt-6 mt-6 border-t border-slate-100 dark:border-slate-800">
            <button class="px-5 py-2.5 text-sm font-bold text-slate-600 dark:text-slate-300 bg-white dark:bg-slate-800 border border-slate-200 dark:border-slate-700 rounded-xl hover:bg-slate-50 dark:hover:bg-slate-700 transition-colors" @click="showDetailModal = false">Close</button>
      ...
      <div v-if="showDeleteConfirm && targetUser" class="fixed inset-0 z-50 flex items-center justify-center">
        ...
        <div class="relative premium-card w-full max-w-sm p-6 sm:p-8 animate-slide-up mx-4 shadow-2xl">
          <div class="text-center">
            <div class="w-16 h-16 rounded-full bg-rose-50 dark:bg-rose-500/10 flex items-center justify-center mx-auto mb-4">
            ...
            <h2 class="text-xl font-extrabold text-slate-900 dark:text-white mb-2">Delete User</h2>
            <p class="text-sm font-medium text-slate-500">
              Are you sure you want to delete <strong>{{ targetUser.fullName }}</strong>? This action cannot be undone.
            </p>
          </div>
          <div class="flex justify-end gap-3 mt-8">
            <button class="flex-1 px-5 py-2.5 text-sm font-bold text-slate-600 dark:text-slate-300 bg-white dark:bg-slate-800 border border-slate-200 dark:border-slate-700 rounded-xl hover:bg-slate-50 dark:hover:bg-slate-700 transition-colors" @click="showDeleteConfirm = false">Cancel</button>
