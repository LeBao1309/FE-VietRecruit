...
<template>
  <div class="max-w-4xl mx-auto px-6 py-8">
    ...
    <!-- Loading -->
    <div v-if="interviewStore.detailLoading" class="space-y-4">
    ...
    <div v-else-if="interview" class="space-y-5">
      <!-- ─── Header Card ─── -->
      <div class="bg-surface border border-border rounded-lg p-6 shadow-sm">
        <div class="flex items-start justify-between mb-4">
          <div>
            <h1 class="text-xl font-bold text-gray-900 mb-2">{{ interview.title }}</h1>
            <div class="flex items-center gap-3 flex-wrap">
              ...
              <span class="text-xs text-gray-400">Created {{ formatDate(interview.createdAt) }}</span>
            </div>
          </div>
          ...
          <div v-if="canManage && interviewStore.isScheduled" class="flex items-center gap-2 shrink-0">
            <button
              ...
              class="px-4 py-2 text-sm font-medium text-white bg-green-600 hover:bg-green-700 rounded-md transition disabled:opacity-50"
            >
              Mark Complete
            </button>
            <button
              ...
              :disabled="interviewStore.statusLoading"
              class="px-4 py-2 text-sm font-medium text-gray-700 bg-surface border border-border rounded-md hover:bg-gray-50 transition disabled:opacity-50"
            >
              Cancel Interview
            </button>
          </div>
        ...
        <div class="grid grid-cols-2 sm:grid-cols-4 gap-4 pt-4 border-t border-border">
          <div>
            <span class="block text-[10px] font-semibold uppercase tracking-wider text-gray-400 mb-1">Scheduled</span>
            <span class="text-sm font-medium text-gray-900">{{ formatDateTime(interview.scheduledAt) }}</span>
          </div>
          <div>
            <span class="block text-[10px] font-semibold uppercase tracking-wider text-gray-400 mb-1">Duration</span>
            <span class="text-sm font-medium text-gray-900">{{ formatDuration(interview.durationMinutes) }}</span>
          </div>
          <div>
            <span class="block text-[10px] font-semibold uppercase tracking-wider text-gray-400 mb-1">Type</span>
            <span class="text-sm font-medium text-gray-900">{{ interview.interviewType ?? '—' }}</span>
          </div>
          <div>
            <span class="block text-[10px] font-semibold uppercase tracking-wider text-gray-400 mb-1">Location / Link</span>
            <a
              v-if="interview.locationOrLink && interview.locationOrLink.startsWith('http')"
              :href="interview.locationOrLink"
              target="_blank"
              rel="noopener"
              class="text-sm text-primary hover:text-primary-hover font-medium transition break-all"
            >
              {{ interview.locationOrLink }}
            </a>
            <span v-else class="text-sm font-medium text-gray-900">{{ interview.locationOrLink ?? '—' }}</span>
          </div>
        </div>
      ...
      <div class="bg-surface border border-border rounded-lg p-6 shadow-sm">
        ...
        <div class="space-y-2">
          <div
            ...
            :key="iv.id"
            class="flex items-center gap-3 p-3 bg-gray-50 rounded-lg border border-border"
          >
            ...
            <div class="flex-1 min-w-0">
              <span class="text-sm font-medium text-gray-900 block">{{ iv.fullName }}</span>
              <span class="text-xs text-gray-400">{{ iv.email }}</span>
            </div>
          </div>
        </div>
      </div>
      ...
      <div class="bg-surface border border-border rounded-lg p-6 shadow-sm">
        <div class="flex items-center justify-between mb-4">
          ...
          <div v-if="interviewStore.averageScore !== null" class="flex items-center gap-2">
            ...
            <span class="text-lg font-bold" :class="getScoreColor(interviewStore.averageScore)">
        ...
        <!-- Loading -->
        <div v-if="interviewStore.scorecardsLoading" class="animate-pulse space-y-3">
          <div v-for="i in 2" :key="i" class="h-24 bg-gray-100 rounded" />
        </div>

        <!-- Empty -->
        <div v-else-if="interviewStore.scorecards.length === 0" class="text-sm text-gray-400 text-center py-8">
      ...
      <div class="bg-surface border border-border rounded-lg p-6 shadow-sm">
        <div class="flex items-center justify-between mb-4">
          <h2 class="text-sm font-semibold text-gray-900">AI Interview Questions</h2>
          <div class="flex items-center gap-2">
            <button
            ...
            >
              <span v-if="interviewStore.generateQuestionsLoading" class="inline-block w-3 h-3 border-2 border-primary/30 border-t-primary rounded-full animate-spin" />
    ...
    <!-- ─── Confirmation Modal ─── -->
    <Teleport to="body">
      <div v-if="showConfirm && confirmAction" class="fixed inset-0 z-50 flex items-center justify-center">
        ...
        <div class="relative bg-surface rounded-lg shadow-xl border border-border w-full max-w-sm p-6 animate-slide-up">
          <div class="text-center">
            <div
              ...
              :class="confirmAction === 'complete' ? 'bg-success-bg text-success' : 'bg-gray-100 text-gray-500'"
            >
            ...
            <div class="flex justify-center gap-2">
              ...
              <button
                ...
                :class="confirmAction === 'complete' ? 'bg-green-600 hover:bg-green-700' : 'bg-gray-600 hover:bg-gray-700'"
              >
