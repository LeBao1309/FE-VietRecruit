...
<template>
  <div class="flex flex-col md:flex-row min-h-screen">
    ...
    <aside
      ...
      class="w-full md:w-60 bg-surface dark:bg-gray-900 border-b md:border-b-0 md:border-r border-border dark:border-gray-700/40 flex-col shrink-0"
    >
      <!-- Logo -->
      <div class="hidden md:flex items-center gap-3 px-5 h-[60px] border-b border-border dark:border-gray-700/40 shrink-0">
        <div class="w-7 h-7 rounded-lg bg-[#009898]/12 flex items-center justify-center shrink-0">
          <LayoutDashboard class="w-3.5 h-3.5 text-[#009898]" />
        </div>
        <router-link
      ...
      <nav class="flex-1 px-3 py-4 flex flex-col gap-0.5 overflow-y-auto">
        ...
        <div class="mt-4 mb-1 px-3">
          <span class="text-[10px] font-bold uppercase tracking-widest text-slate-400 dark:text-slate-500">Find Jobs</span>
        </div>
        <router-link
        ...
        <div class="mt-4 mb-1 px-3">
          <span class="text-[10px] font-bold uppercase tracking-widest text-slate-400 dark:text-slate-500">Applications</span>
        </div>
        <router-link
        ...
        <!-- Profile section -->
        <div class="mt-4 mb-1 px-3">
          <span class="text-[10px] font-bold uppercase tracking-widest text-slate-400 dark:text-slate-500">Profile</span>
        </div>
        <router-link
        ...
        <router-link
          ...
          active-class="!bg-[#009898]/10 !border-l-[#009898] !text-[#007070] !font-semibold"
        >
          <FilePen class="w-4 h-4 shrink-0" />
          CV Management
        </router-link>
      ...
      <!-- User footer -->
      <div class="px-3 py-3 border-t border-border dark:border-gray-700/40 bg-slate-50/40 dark:bg-white/3 shrink-0">
        <div class="flex items-center gap-2.5 px-2">
          <div class="w-7 h-7 rounded-full bg-[#009898]/15 text-primary-hover flex items-center justify-center text-xs font-bold shrink-0 ring-1 ring-[#009898]/20">
            {{ auth.user?.fullName?.charAt(0)?.toUpperCase() ?? 'C' }}
          </div>
          <div class="flex flex-col min-w-0 flex-1">
            <span class="text-xs font-semibold text-slate-700 dark:text-slate-200 truncate leading-tight">{{ auth.user?.fullName ?? 'Candidate' }}</span>
            <span class="text-[10px] text-slate-400 dark:text-slate-500 leading-tight mt-0.5">Candidate</span>
          </div>
          <button
            @click="auth.logout()"
