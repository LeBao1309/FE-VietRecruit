...
<template>
 ...
 :class="isRejected ? 'text-rose-500' : isWithdrawn ? 'text-slate-400' : 'text-slate-500 '"
 >
 {{ currentLabel }}
 </span>
 <span v-if="!isRejected && !isWithdrawn" class="text-[10px] font-bold text-teal-600 ">
 ...
 ? 'bg-slate-300 '
 : index <= activeIndex 
 ? (index === activeIndex ? 'bg-teal-500 shadow-[0_0_8px_rgba(20,184,166,0.5)]' : 'bg-teal-500/50 ')
 : 'bg-slate-200 '
 ]"
 />
 </div>
